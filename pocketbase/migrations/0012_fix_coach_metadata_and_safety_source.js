...
// Corrige a fonte de segurança bloqueada e restaura metadados do Coach IA.
// O prompt atual é preservado diretamente do registro para evitar divergência.
migrate(
  (app) => {
    const current = app.findFirstRecordByFilter('_agents', 'slug = "fitness-coach"')
    const systemPrompt = current.getString('system_prompt')
